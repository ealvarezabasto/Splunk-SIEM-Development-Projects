...
*    1. Configure receiving on a Splunk Enterprise instance (e.g., Search Head server)
...
* **Note** For the logs to be sent from the Universal Forwarder, there needs to be a receiving port at the Splunk site (e.g., Search Head server or Indexer server) (Port 9997).
...
* Type root@157.230.11.23 (Splunk UF Public IP Address) > paste the wget link
...
* Login to Search Head server via the UI

* Within ***Search & Reporting*** app, view the various logs that are successfully being sent from the var/log directory.
